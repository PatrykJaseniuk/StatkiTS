...
export class Ship2 {
    ...
    constructor() {
        ...
        // this whole long garbage code was generated by GPT but it woks
        //turn sail Front
        //left
        this.userInteractor.addTrigerAction(
            {
                triger: (pointer) => {
                    const triger =
                        pointer &&
                        pointer.position.value.clone().sub(this.sail1.mast.position.value).length() < 100 &&
                        pointer.isLMBDown;

                    return triger == true;
                },
                action: () => {
                    this.turnSail('front', Math.PI / 2000 / timeSpeed.value);
                }
            }
        )
        ...
        this.userInteractor.addTrigerAction(
            {
                triger: (pointer) => {
                    const triger =
                        pointer &&
                        pointer.position.value.clone().sub(this.sail2.mast.position.value).length() < 100 &&
                        pointer.isLMBDown;
                ...
                ,
                action: () => {
                    this.turnSail('back', Math.PI / 2000 / timeSpeed.value);
                }
            });
        ...
        this.userInteractor.addTrigerAction(
            {
                triger: (pointer) => {
                    const triger =
                        pointer &&
                        pointer.position.value.clone().sub(this.ster.dynamicElement.position.value).length() < 100 &&
                        pointer.isLMBDown;
                ...
                ,
                action: () => {
                    this.ster.rotationOfSter.value += Math.PI / 30000 / timeSpeed.value;
                }
            });
